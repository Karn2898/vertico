...
def index_repo(
    ...
    with Session(engine) as db_session:
        ...
        if repo.is_indexed(str(root)):
            logger.info(f"Repo already indexed: {root}. Use force_reindex=True to reindex")
        ...
        embeddings = embed_batch(
            texts,
            on_progress=lambda done, total: logger.info(f"embedded {done}/{total}"),
        )
